  ...
  <div class="container-fluid px-4 d-flex align-items-center justify-content-between">
    <div class="d-flex align-items-center gap-3">
      <a href="index.php" class="navbar-brand fw-extrabold text-dark-contrast mb-0 d-flex align-items-center gap-2">
        <div class="bg-gradient-success text-white rounded-3 p-2 d-inline-flex align-items-center justify-content-center shadow-sm" style="width: 38px; height: 38px;">
          <i class="fas fa-user-tie"></i>
        </div>
        <span class="fs-4">Digital <span class="gradient-text-mask">Internship</span></span>
      </a>
      <span class="badge badge-adv badge-adv-success px-3 py-2">Supervisor Portal</span>
    </div>
  ...
  <div class="row g-4">
    ...
    <div class="col-md-3 col-lg-2">
      <div class="adv-card mb-4 p-3 text-center">
        ...
        <h6 class="fw-extrabold mb-0 text-dark-contrast"><?php echo htmlspecialchars($currentSup['name']); ?></h6>
        <small class="text-dark-contrast font-weight-bold extra-small d-block text-break"><?php echo htmlspecialchars($currentSup['email']); ?></small>
    ...
    <div class="col-md-9 col-lg-10">
      ...
      <div id="tab-sup-tasks" class="tab-content">
        <div class="adv-card p-4 mb-4">
          ...
          <form onsubmit="assignTaskToStudent(event)">
            <div class="row g-3 mb-3">
              <div class="col-md-6">
                <label class="form-label font-weight-bold text-dark-contrast">Select Intern Student</label>
                <select id="task-student-select" required class="form-select py-2"></select>
              </div>
              <div class="col-md-6">
                <label class="form-label font-weight-bold text-dark-contrast">Task Completion Deadline</label>
                <input type="date" id="task-deadline" required class="form-control py-2">
              </div>
            </div>
            <div class="mb-3">
              <label class="form-label font-weight-bold text-dark-contrast">Task Title</label>
              <input type="text" id="task-title" required class="form-control py-2" placeholder="e.g. Implement Responsive Navigation Component">
            </div>
            <div class="mb-3">
              <label class="form-label font-weight-bold text-dark-contrast">Task Instructions</label>
        ...
        <h3 class="fw-extrabold text-dark-contrast mb-3">Issued Tasks Overview</h3>
      ...
      <div id="tab-sup-reports" class="tab-content d-none">
        <h3 class="fw-extrabold text-dark-contrast mb-3">Review Progress Reports</h3>
  ...
  <div class="modal-dialog">
    <div class="modal-content rounded-4 border-0">
      <div class="modal-header bg-gradient-success text-white border-0 py-3">
        <h5 class="modal-title font-weight-extrabold text-white">Evaluate Report</h5>
      ...
      <form onsubmit="saveReportEvaluation(event)">
        <div class="modal-body p-4">
          ...
          <div class="mb-3">
            <label class="form-label font-weight-bold text-dark-contrast">Rating (1 - 5 Stars)</label>
          ...
          <div class="mb-3">
            <label class="form-label font-weight-bold text-dark-contrast">Supervisor Feedback</label>
        ...
        <div class="modal-footer border-0">
          ...
          <button type="submit" class="btn btn-adv-primary btn-sm px-4 font-weight-bold">Submit Evaluation</button>
        </div>
      </form>
    </div>
  </div>
...
<script>
  ...
  function renderIssuedTasks() {
    ...
    if (tasks.length === 0) {
      grid.innerHTML = '<div class="col-12 text-center py-4 text-dark-contrast font-weight-bold">No tasks issued yet.</div>';
      return;
    }
        ...
        <div class="adv-card p-4 h-100 d-flex flex-column justify-content-between">
          <div>
            <div class="d-flex justify-content-between mb-2">
              <small class="text-dark-contrast font-weight-bold"><i class="far fa-clock me-1 text-primary"></i> Deadline: ${t.deadline}</small>
              <span class="badge badge-adv ${isDone ? 'badge-adv-success' : 'badge-adv-warning'}">${t.status}</span>
            </div>
            <h5 class="font-weight-bold text-dark-contrast mb-2">${t.title}</h5>
            <p class="text-dark-contrast font-weight-medium small">${t.description}</p>
          </div>
        </div>
      `;
  ...
  function renderSubmittedReports() {
    ...
    if (reports.length === 0) {
      container.innerHTML = '<div class="text-center py-4 text-dark-contrast font-weight-bold">No reports submitted.</div>';
      return;
    }
    ...
    reports.forEach(r => {
      ...
      card.innerHTML = `
        <div class="d-flex justify-content-between mb-2">
          <h5 class="font-weight-extrabold text-primary mb-0">Week ${r.weekNumber} Report - Student (${r.studentId})</h5>
        ...
        <p class="small text-dark-contrast font-weight-semibold mb-3"><strong>Achievements:</strong> ${r.achievements}</p>
        ${r.rating ? `
          <div class="bg-light p-3 rounded-3 border border-success extra-small text-dark-contrast font-weight-bold">
            ...
            <div class="text-dark-contrast mt-1"><strong>Supervisor Feedback:</strong> ${r.feedback}</div>
          </div>
        ` : `
          <button onclick="openReviewModal('${r.id}')" class="btn btn-adv-primary btn-sm font-weight-bold">
