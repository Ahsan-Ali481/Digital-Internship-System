<?php
// forgot-password.php - Password Recovery Page with Emojis
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($role === 'admin' || strtolower($email) === 'admin123@gmail.com' || strpos(strtolower($email), 'admin') !== false) {
        $error = "🛡️ Security Policy Notice: Password recovery is NOT allowed for Administrator accounts online. Administrator passwords can only be updated manually by system superuser.";
    } elseif (empty($email) || empty($newPassword)) {
        $error = "Please enter your registered Email address and new Password. ⚠️";
    } else {
        if (isset($pdo)) {
            try {
    ...
    <div class="col-md-6 col-lg-5">
      <div class="card-premium p-0 overflow-hidden shadow-lg">
        <div class="bg-gradient-primary text-white text-center py-4">
          <h3 class="mb-0 font-weight-extrabold text-white"><span class="emoji-icon me-2">🔑</span> Account Password Reset</h3>
        </div>
        <div class="p-4 p-md-5">
          ...
          <form action="forgot-password.php" method="POST">
            <div class="mb-3">
              <label class="form-label font-weight-bold text-dark">🎯 Select Your Account Role</label>
              <select name="role" id="reset-role" class="form-select py-2" onchange="checkAdminPolicy()">
                ...
                <option value="admin">🛡️ System Admin Account (Disabled)</option>
              </select>
              <div id="admin-warning" class="form-text text-danger d-none mt-1 font-weight-bold">
                ⚠️ Password reset is disabled for Admin accounts for security reasons.
              </div>
            </div>

            <div class="mb-3">
              <label class="form-label font-weight-bold text-dark">✉️ Registered Gmail Address</label>
            ...
            <button type="submit" id="reset-btn" class="btn btn-premium-primary w-100 py-3 font-weight-bold shadow-md">
              <span class="emoji-icon me-2">🔄</span> Reset Password & Sign In
            </button>
          </form>
